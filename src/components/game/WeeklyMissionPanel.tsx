...
type WeeklyMissionPanelProps = {
  missions: MissionPreview[];
  periodKey: string;
  onPreviewClaim: (mission: MissionPreview) => void;
};

export function WeeklyMissionPanel({ missions, onPreviewClaim, periodKey }: WeeklyMissionPanelProps) {
  return (
    <GameCard>
      <SectionTitle eyebrow="Weekly Missions" title="Weekly Mission รีเซ็ตวันจันทร์ 00:00 Asia/Bangkok" />
      <p className="mt-2 text-sm text-slate-300">Bangkok week key: {periodKey}</p>
      <div className="mt-4 grid gap-3">
        {missions.map((mission) => (
          <article className="rounded-xl border border-white/10 bg-white/[0.03] p-3" key={mission.mission_id}>
            ...
            <p className="mt-2 text-xs text-slate-300">Mission Reward ใช้ reward_snapshot</p>
            <p className="mt-1 text-xs text-slate-500">snapshot: {mission.reward_snapshot.reward_snapshot_id}</p>
            <button
              className="mt-3 rounded-xl border border-sky-300/30 bg-sky-400/10 px-3 py-2 text-xs font-bold text-sky-100 disabled:opacity-50"
